...
import { AdminService, OutlookService,NewsService } from "../services/api";
...
const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  setOutlooks,
  mentorInstruction,
  setMentorInstruction,
}) => {
  const [activeTab, setActiveTab] = useState<
    "reviews" | "outlook" | "access" | "translations"
  >("reviews");
  ...
  const handleImportNews = async () => {
      if (!editedOutlook.date) {
          alert("Please set a date for the outlook first.");
          return;
      }
      
      const newsEvents = await NewsService.getHighImpactEvents(editedOutlook.date);
      ...
      setEditedOutlook(prev => ({
          ...
          timeline: [
              ...prev.timeline,
              ...newsEvents
          ].sort((a, b) => a.label.localeCompare(b.label)) // Sort by time
      }));
      
      alert(`Imported ${newsEvents.length} high-impact events.`);
  };
  ...
  // --- OUTLOOK EDIT LOGIC ---
  const [selectedOutlookType, setSelectedOutlookType] =
    useState<OutlookType>("daily");
  ...
  const filteredOutlooks = useMemo(() => {
    return outlooks
      .filter((o) => o.type === selectedOutlookType)
  ...
  const [editedOutlook, setEditedOutlook] =
    useState<MentorOutlook>(emptyOutlook);
  ...
  const saveOutlook = async () => {
    ...
    if (isNew) {
      ...
      setOutlooks(
        outlooks.map((o) => (o.id === outlookToSave.id ? outlookToSave : o))
      );
    }
    ...
    switch (preset) {
      ...
      case "lastWeek":
        const prevMonday = new Date(
          today - (7 + (new Date(today).getDay() || 7) - 1) * msInDay
        );
  ...
  const filteredTrades = useMemo(() => {
    ...
    if (strategyFilter !== "all")
      data = data.filter((t) => t.strategy === strategyFilter);

    // Timeframe
    if (timeframeFilter !== "all")
      data = data.filter((t) => t.timeframe === timeframeFilter);
    ...
    setSelectedTrade((prev) =>
      prev
        ? {
            ...
            status,
            adminFeedback: reviewNote,
            adminFeedbackAudio: audioBase64 || prev.adminFeedbackAudio,
          }
        : null
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Manage reviews and market outlooks
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center bg-slate-800 p-4 rounded-xl">
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={addLanguage}
                className="text-sm font-bold text-gold-500 flex items-center gap-1 hover:text-white"
              >
            ...
            <div className="flex gap-2 items-center">
              ...
              >
                <Save size={18} />{" "}
                {isSavingTrans ? "Saving..." : "Save Changes"}
              </button>
            </div>
          </div>
          ...
          <div className="glass-panel p-4 rounded-xl overflow-hidden">
            <div className="max-h-[70vh] overflow-y-auto space-y-1 pr-2">
              {Object.keys(localTranslations["en"] || {})
                .filter(
                  (k) =>
                    k.toLowerCase().includes(searchTrans.toLowerCase()) ||
                    (localTranslations[editingLang]?.[k] || "")
                      .toLowerCase()
                      .includes(searchTrans.toLowerCase())
                )
                .map((key) => (
                  <div
                    key={key}
                    className="grid grid-cols-3 gap-4 items-center p-3 border-b border-slate-800 hover:bg-slate-800/30"
                  >
            ...
            {showPersonaConfig && (
              <div className="mt-4 animate-in slide-in-from-top-2">
                ...
                <p className="text-[10px] text-slate-500 mt-2">
                  These instructions will guide Gemini when generating automatic
                  trade reviews.
                </p>
              </div>
            )}
          ...
          <div className="glass-panel p-4 rounded-xl flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center">
            <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto items-center">
              ...
              <div className="relative group w-full md:w-auto md:min-w-[200px]">
                <Search
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-gold-500 transition-colors"
                  size={16}
                />
                <input
                  type="text"
              ...
              <div className="flex bg-dark-900 p-1 rounded-lg border border-slate-700">
                {(["pending", "reviewed", "rejected", "all"] as const).map(
                  (s) => (
                    <button
                      key={s}
            ...
            <div className="glass-panel p-4 rounded-xl animate-in slide-in-from-top-2">
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded border border-red-900/50 text-red-400 text-xs font-bold uppercase hover:bg-red-900/20"
                >
                  Clear
                </button>
              </div>
              <div className="flex gap-2 items-center text-xs text-slate-400">
                ...
                <input
                  type="date"
                  className="bg-dark-900 border border-slate-700 rounded p-1 text-white"
                  onChange={(e) =>
                    setDateRange((prev) => ({
                      ...prev,
            ...
            <div className="space-y-4">
              ...
              {filteredTrades.map((trade) => (
                <div
                  ...
                  className={`glass-panel p-4 rounded-xl cursor-pointer border-l-4 transition-all ${
                    selectedTrade?.id === trade.id
                      ? "border-l-gold-500 bg-slate-800"
                      : "border-l-transparent hover:border-l-gold-500/50 hover:bg-slate-800/50"
                  }`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {trade.status === TradeStatus.REVIEWED && (
            ...
            <div className="glass-panel p-6 rounded-2xl h-fit sticky top-6 overflow-y-auto max-h-[85vh]">
              {selectedTrade ? (
                <div className="space-y-6">
                  <div className="border-b border-slate-700 pb-4 flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        ID: {selectedTrade.id} • {selectedTrade.userName}
                      </p>
                    </div>
                    <div
                      className={`px-3 py-1 rounded text-xs font-bold uppercase ${
                        selectedTrade.status === "reviewed"
                          ? "bg-green-500/20 text-green-400"
                      ...
                      <div className="flex items-center gap-3">
                        {isRecording ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <audio
                              src={selectedTrade.adminFeedbackAudio}
                              controls
                              className="h-8 w-32"
                            />
                          </div>
                          ...
                          <span className="text-slate-500 text-xs">
                            Record voice feedback (optional)
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        {!isRecording && !audioBlobUrl && (
                          <button
                            ...
                            className="p-2 bg-slate-800 text-slate-300 rounded-lg hover:bg-gold-500 hover:text-dark-900 transition-colors"
                          >
                            <Mic size={18} />
                          </button>
                        )}
                        ...
                        {audioBlobUrl && (
                          <button
                            onClick={resetAudio}
                            className="p-2 bg-slate-800 text-red-400 rounded-lg hover:bg-slate-700 transition-colors"
                          >
                            <Trash2 size={18} />
                          </button>
                        )}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      onClick={() => updateStatus(TradeStatus.REJECTED)}
                      className={`py-3 rounded-xl font-bold flex justify-center items-center gap-2 transition-all ${
                        selectedTrade.status === "rejected"
                    ...
                    <button
                      onClick={() => updateStatus(TradeStatus.REVIEWED)}
                      className={`py-3 rounded-xl font-bold flex justify-center items-center gap-2 transition-all ${
                        selectedTrade.status === "reviewed"
                    ...
                    <button
                      onClick={() => updateStatus(TradeStatus.PENDING)}
                      className={`col-span-2 py-2 rounded-xl text-sm font-bold flex justify-center items-center gap-2 transition-all ${
          ...
          <div className="glass-panel p-6 rounded-2xl border-t-4 border-gold-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  Create or edit market briefings
                </p>
              </div>

              <div className="flex flex-wrap gap-2">
                {(["session", "daily", "weekly", "monthly"] as const).map(
                  (type) => (
                    <button
                      key={type}
                      onClick={() => {
                        setSelectedOutlookType(type);
                        setEditingOutlookId("new");
                      }}
            ...
            <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
              <div className="relative w-full md:w-64">
                ...
                >
                  <option value="new">
                    + Create New{" "}
                    {selectedOutlookType.charAt(0).toUpperCase() +
                      selectedOutlookType.slice(1)}
                  </option>
              ...
              <div className="flex gap-2">
                ...
                {editingOutlookId !== "new" && (
                  <button
                    onClick={deleteOutlook}
                    className="bg-red-500/10 hover:bg-red-500/20 text-red-500 px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors border border-red-500/50"
                  >
                    <Trash2 size={18} />
                  </button>
                )}
            ...
            <div className="space-y-6">
              ...
              {selectedOutlookType === "session" && (
                <div className="glass-panel p-4 rounded-xl border-dashed border border-slate-600 bg-slate-800/20">
                  <label className="block text-xs font-bold text-gold-500 uppercase mb-3">
                    Session Configuration
                  </label>
                  <div className="flex gap-2 mb-4">
                    {(["Asia", "London", "New York"] as const).map(
                      (session) => (
                        <button
                          key={session}
                          onClick={() => handleSessionChange(session)}
                          className={`flex-1 py-3 rounded-xl border text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                            editedOutlook.sessionName === session
                              ? "bg-gold-500 text-dark-900 border-gold-500"
              ...
              {(selectedOutlookType === "weekly" ||
                selectedOutlookType === "monthly") && (
                <div className="glass-panel p-4 rounded-xl border-dashed border border-slate-600 bg-slate-800/20">
                  <label className="block text-xs font-bold text-gold-500 uppercase mb-3">
                    Period Duration
                  </label>
              ...
              <div className="glass-panel p-4 rounded-xl border border-slate-700/50 bg-slate-800/20 mt-4">
                ...
                <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                  {(editedOutlook.notes || []).map((note, i) => (
                    <div
                      key={i}
                      className="flex gap-2 items-start bg-dark-900 p-3 rounded-lg border border-slate-700"
                    >
                  ...
                  {(!editedOutlook.notes ||
                    editedOutlook.notes.length === 0) && (
                    <p className="text-center text-slate-600 text-xs italic py-2">
                      No live updates added.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="glass-panel p-4 rounded-xl border border-slate-700/50 bg-slate-800/20">
                ...
                <div className="space-y-3">
                  ...
                  {editedOutlook.levels.map((level, i) => (
                    <div
                      key={i}
                      className="flex gap-2 items-center bg-dark-900 p-2 rounded-lg border border-slate-700"
                    >
                      <input
                        type="number"
                        placeholder="Price"
             ...
             <div className="flex justify-between items-center mb-3">
                            ...
                            <div className="flex gap-2">
                                {/* NEW BUTTON: IMPORT NEWS */}
                                <button 
                                    onClick={handleImportNews}
                                    className="text-xs bg-red-500/10 text-red-500 border border-red-500/20 px-2 py-1 rounded hover:bg-red-500/20 flex items-center gap-1 font-bold transition-colors"
                                    title="Import High Impact News for this date"
                                >
                                    <Radio size={12}/> Auto-Import News
                                </button>
                    ...
                    <div key={i} className="flex gap-2 items-center">
                      ...
                      {item.type === 'news' && (
                                        <div className="flex items-center gap-1">
                                             <span className="text-[9px] text-slate-500 font-mono bg-dark-900 px-1 rounded">{item.currency}</span>
                                        </div>
                                    )}
          ...
          <div className="glass-panel rounded-2xl overflow-hidden">
            ...
            {tokens.length === 0 ? (
              ...
              tokens.map((token, idx) => (
                <div
                  key={idx}
                  className="grid grid-cols-12 gap-4 p-4 border-t border-slate-800 items-center text-sm hover:bg-slate-800/30 transition-colors"
                >
